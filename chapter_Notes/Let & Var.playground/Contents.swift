...
var newName: String = "Anakin"
newName = "Darth Vader"  // Changing the "name" variable to another string is allowed...
// ...but trying to change the var to a value of ANOTHER type is not allowed.
//NewName = 42             // THIS WON'T WORK because we are trying to change a variable that is a "String"
...
var newerName = "Anakin"  // String type
...
//We declare constants using the "let"                             // Declare name as a constant because we anticipate that it will NOT change.
let NewestName: String = "Anakin Skywalker"
// Note that just like variables, constants also have types
// Now we cannot change the name:
//newestName = "Darth Vader"        // THIS WON'T WORK because name is a constant so we CANNOT change it.


//  Mutable vs. Immutable
//Mutable means that we can change the value and immutable means that we cannot change the value. In Objective-C, there were explicit Types that were indicated as Immutable. For example, there was a class for mutable String Types and a separate class for immutable String Types. In Swift, we can still have immutable and mutable Strings without having another class because of the difference between the let and the var keywords.

//We use the keyword let when we want to declare a constant. Constants are values that are not going to change during your program. We use the var keyword when we want to hold onto a value that might change. Using these keywords we can create mutable strings and immutable strings.
