...
//We create the Person class that has a "fullName" property and an introduce method
//We create an instance of Person and assign it to the "j" variable. This instance has the value "Ketul Patel" stored in the fullName property
//We create another variable "k" and assign it to the "j" variable. Since classes are reference types instead of copying the instance in the "j" variable we simply point both "k" and "j" to the same instance in memory
//Since k and j refer to the same instance, when we change the value by referencing the instance through the k variable it changes the value for j as well (one instance in memory referred to by 2 variables or "pointers")
