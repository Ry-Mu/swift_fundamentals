...
//Here we declared the variable  xFactor to be of an Optional Type that may contain an instance of a String Type. We are declaring this as an optional because xFactor might potentially be nil and we should not assume xFactor to contain a value in our program. Let's store a value to this Optional Type and try to print it out(see above).
...
var newpresent: String? = "Apple Watch"        // If we can let "unwrappedPresent" equal "present"
// (meaning there is something inside of present)
// then go on to do something with "unwrappedPresent"
print("OMG THANK YOU FOR THE \(newpresent!)")  // Force unwrap using the "!"
...
var newerpresent: String! = "Apple Watch"         // We don't have to unwrap to use the value,
print("\(newerpresent)")
newerpresent = nil                                // but we can still set it to nil.
print("\(newerpresent)")
