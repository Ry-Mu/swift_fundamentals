...
class Deck {
    ...
    func shuffle(){
        for i in stride(from: self.cards.count-1, to: 0, by: -1){
            ...
            let swapCardIndex = Int(arc4random_uniform(UInt32(i))) //need to ask about how this works
            let temp = self.cards[i]
            self.cards[i] = self.cards[swapCardIndex] //I get why we swap, but why set the tempm again after this line?
...
class Player {
    ...
    func draw(deck:Deck) -> Card? {
        ...
        if let drawnCard = deck.deal() {  //why if let again?
            //We'll utiilize the deck's deal method
    ...
    func discard(suit: String, value: String) -> Bool { //why not use cards?
        ...
        for i in 0...self.hand.count-1 {
            if (self.hand[i].suit == suit && self.hand[i].value == value) { //need an explanation for this
...
//You can check to make sure everything's been shuffled by calling the show() again
print("<<<<<<<<<<<<<<< New deck was made >>>>>>>>>>>>>>>>>")
newDeck.show()

//Make a new player and name him Joe
